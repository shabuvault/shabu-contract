...
contract ShabuVaultCore is ERC20("ShabuVault", "SHABU"), Ownable {
    using SafeMath for uint256;
...
contract ShabuVault is ShabuVaultCore {
...
contract LPToSHABU is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for ShabuVault;
    using SafeERC20 for IERC20;
    using Address for address;

    ShabuVault private shabu;
    ...
    // initial amount of shabu
    uint256 public initReward = 7250 * 1e18;
    ...
    uint256 public devFundAmount = 1000 * 1e18;
    ...
    constructor(address _shabu, address _lpToken) {
        shabu = ShabuVault(_shabu);
    ...
    function claim(uint amount) public shouldStarted {
        ...
        shabu.safeTransfer(msg.sender, amount);
        distributeDevFund();
    ...
    function checkHalving() internal {
        if (block.timestamp >= halvingTime) {
            ...
            shabu.mint(address(this), initReward);
    ...
    function startFarming() external onlyOwner {
        ...
        shabu.mint(address(this), mintAmount);
    ...
    function distributeDevFund() internal {
        ...
        shabu.safeTransfer(devAddr, fundAmount);
    ...
    function shabuAddress() view public returns(address) {
        return address(shabu);
    }

    function testMint() public onlyOwner {
        shabu.mint(address(this), 1);
    }
}